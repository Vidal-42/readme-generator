...
def capturar_estrutura_python(diretorio_alvo="."):
    """Gera uma árvore de diretórios simplificada (ignora .git, __pycache__, .venv)."""
    base = Path(diretorio_alvo).resolve()
    ignorar = {'.git', '__pycache__', '.venv', 'venv', '.pytest_cache', '.vscode', '.amazonq'}
    linhas = []

    for root in sorted(base.rglob('*')):
        if any(part in ignorar for part in root.parts):
            continue
        if root.is_dir():
            nivel = len(root.relative_to(base).parents) - 1
            indent = '    ' * nivel
            linhas.append(f"{indent}|-- {root.name}/")
        else:
            if root.suffix == '.py':
                nivel = len(root.relative_to(base).parents)
                indent = '    ' * nivel
                linhas.append(f"{indent}|-- {root.name}")
    return "\n".join(linhas)
